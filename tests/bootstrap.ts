...
export const runnerHooks: Required<Pick<Config, 'setup' | 'teardown'>> = {
  setup: [
    ...
    () => {
      execaCommandSync(`mkdir -p ${join(__dirname, '..', 'tmp')}`)
      execaCommandSync('node ace migration:run')
    },
  ],
  teardown: [
    () => execaCommandSync('node ace migration:rollback'),
    () => rimraf(join(__dirname, '..', 'tmp', 'db.sqlite3'), {}, () => {}),
  ],
}

...
test.group('tickets/count [GET]', ({ each }) => {
  each.setup(async () => {
  ...
  test('/tickets/count [GET]', async ({ client }) => {
    const response = await client.get('/tickets/count').guard('api').loginAs(admin.user).send()

    response.assertBodyContains({ open: Number, solving: Number, total: Number })
